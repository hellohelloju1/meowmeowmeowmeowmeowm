...
void opcontrol() {
		...
		while (true) {
			...
			float leftx = (float) master.get_analog(pros::E_CONTROLLER_ANALOG_LEFT_X) / 127 * 200;
			float righty = (float) master.get_analog(pros::E_CONTROLLER_ANALOG_RIGHT_Y) / 127 * 200;
			float rightx = (float) master.get_analog(pros::E_CONTROLLER_ANALOG_RIGHT_X) / 127 * 200;
			pros::lcd::set_text(1, std::to_string(lefty));
			pros::lcd::set_text(2, std::to_string(leftx));
			if (lefty<LIMIT_TOP && LIMIT_BOTTOMHAHA<lefty) {
			MotorLeft.brake();
			MotorRight.brake();
			} else if (lefty>LIMIT_TOP){
			MotorLeft.move_velocity(lefty+leftx);
			MotorRight.move_velocity(lefty - leftx);
